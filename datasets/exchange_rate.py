...
class Dataset(BaseDataset):
    ...
    def get_data(self):
        ...
        if not os.path.exists(os.path.join(data_path, "exchange_rate.csv")):
            url = (
                "https://drive.google.com/uc?&id=1EBLfP2"
                "Dx2K7LsSZybX4JJes-wEcTJbz-&export=download"
            )

            response = requests.get(url)
            with open(os.path.join(data_path, "exchange_rate.csv"), "wb") as f:
